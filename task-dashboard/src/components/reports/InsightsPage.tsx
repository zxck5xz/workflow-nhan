      ...
      <div className="scorecard-grid">
        {scorecards.length === 0 && (
          <div className="empty-state">
            Chưa có bảng điểm nào. Hãy bắt đầu chấm điểm chất lượng game!
          </div>
        )}
    ...
    <div className="rating-row">
      ...
      <div className="rating-bar-container">
        <div
          ...
          style={{ width: `${(value / 5) * 100}%`, background: getRatingColor(value) }}
        />
      </div>
  ...
  return (
    <div className="insights-section">
      ...
      <div className="insight-list">
        {insights.length === 0 && (
          <div className="empty-state">
            Chưa có bản tổng hợp nào. Hãy tạo insight để tracking sức khỏe dự án!
          </div>
        )}
        {insights.map((ins: WeeklyInsight) => (
          <div key={ins.id} className="insight-card card">
            ...
            <div className="card-actions">
              <Button variant="ghost" size="sm" onClick={() => onEdit(ins)}>
                ✏️
              </Button>
              <Button variant="ghost" size="sm" onClick={() => onDelete(ins.id)}>
                🗑️
              </Button>
            </div>
          </div>
...
function ScorecardModal({ isOpen, scorecard, projects, members, onClose, onSave }: any) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={scorecard ? 'Sửa bảng điểm' : 'Chấm điểm Game'}
      footer={
        <>
    ...
    >
      ...
      <div className="form-group">
        ...
        <textarea
          ...
          placeholder="Điểm mạnh, điểm yếu, cơ hội cải thiện..."
        />
      </div>
    </Modal>
  );
...
function InsightModal({ isOpen, insight, members, onClose, onSave }: any) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={insight ? 'Sửa Insight' : 'Tạo Insight tuần'}
      footer={
        <>
          <Button onClick={onClose}>Hủy</Button>
          <Button variant="primary" onClick={handleSubmit}>
    ...
    >
      <div className="form-group">
        ...
        <input
          ...
          placeholder="VD: Tổng kết tuần 20 - Focus Battle Arena"
        />
      </div>
      <div className="form-group">
    ...
    const logInterval = setInterval(() => {
      ...
      if (stepCount === 1) {
        setTerminalLogs((prev) => [
          ...prev,
          '> [INFO] Đang tìm kiếm gameplay review & phân tích đối thủ cạnh tranh...',
        ]);
      } else if (stepCount === 2) {
        setTerminalLogs((prev) => [
          ...prev,
          `> [INFO] Thể loại: ${genre} | Đối thủ: ${competitors || 'N/A'}`,
        ]);
      } else if (stepCount === 3) {
        setTerminalLogs((prev) => [
          ...prev,
          '> [INFO] Đang phân tích sắc thái nhận xét & tự động chấm điểm...',
        ]);
      } else if (stepCount === 4) {
        ...
        setTerminalLogs((prev) => [
          ...prev,
          '> [INFO] Đang biên dịch báo cáo đánh giá dạng Markdown...',
        ]);
      }
    }, 1200);
        ...
        <div className="eval-setup-layout">
          <div className="eval-form card">
            ...
            <div className="form-group">
              <label className="form-label">
                Tên Game <span className="required-star">*</span>
              </label>
              <input
                type="text"
                className="form-input"
          ...
          <div className="criteria-manager card">
            ...
            <p className="description">
              Chọn các hạng mục cốt lõi và thêm bớt các tiêu chí tùy chỉnh để AI Agent thực hiện
              chấm điểm.
            </p>
        ...
        <div className="eval-results-container animate-scale-in">
          ...
          {showRawText ? (
            <div className="raw-markdown-view card">
              ...
              <textarea className="code-textarea" readOnly value={reportMarkdown} />
            </div>
          ) : (
...
function SlideContentRenderer({ markdown }: SlideContentRendererProps) {
      ...
      if (ratingMatch) {
        ...
        bodyContent.push(
          <div key={`sub-${idx}`} className="slide-section-rating">
            ...
            <div
              className="section-rating-score-pill"
              style={{ background: `${getRatingColor(score)}20`, color: getRatingColor(score) }}
            >
              {score} / 5.0
            </div>
          </div>,
        ...
        bodyContent.push(
          <h3 key={`sub-${idx}`} className="slide-sub-title">
            {text}
          </h3>,
        );
      }
  ...
  return (
    <div className="slide-canvas">
      ...
      <div className="slide-body">{bodyContent}</div>
    </div>
  );
}
...
function HelpModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Hướng dẫn sử dụng tab Insights"
      footer={<Button onClick={onClose}>Đóng</Button>}
    >
      <div className="insights-help-content">
        <h4>Tổng quan</h4>
        <p>
          Tab Insights gồm 3 phần chính: <strong>Game Scorecards</strong>,{' '}
          <strong>Tổng kết tuần</strong> và <strong>AI Đánh giá</strong>. Dưới đây là cách sử dụng
          nhanh.
        </p>
        ...
        <ul>
          <li>
            Nhấn <strong>+ Chấm điểm mới</strong> để tạo bảng điểm cho từng dự án/tuần.
          </li>
        ...
        <ul>
          <li>
            Nhấn <strong>+ Tạo Insight mới</strong> để thêm bản tổng hợp tuần.
          </li>
          <li>
            Chia thành <em>Highlights</em>, <em>Risks</em> và <em>Action Items</em> — mỗi dòng là
            một mục.
          </li>
        ...
        <ul>
          <li>
            Nhập tên game, thể loại, đối thủ và thông tin bổ trợ rồi nhấn{' '}
            <strong>⚡ Bắt đầu Đánh giá</strong>.
          </li>
        ...
        <ul>
          ...
          <li>
            Lưu ý nhập rõ ràng hành động trong <em>Action Items</em> để dễ follow-up.
          </li>
        </ul>
      </div>
    </Modal>
