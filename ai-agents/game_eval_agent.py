...
class GameEvalAgent:
    ...
    def is_valid_row(self, heading, desc):
        ...
        return not any(x in heading_lower for x in metadata_keywords)
    ...
    def load_from_xlsx(self, file_path):
        ...
        if file_path.lower().endswith(".xls"):
            is_xlsx = bool(self.is_zip_file(file_path))
        ...
        try:
            if is_xlsx:
                ...
                print(
                    f"Loaded Excel sheets: {sheet_names} from {os.path.basename(file_path)} (using openpyxl)"
                )
                ...
                if "Testing Report" in sheet_names and "SWOT" in sheet_names:
                    self.parse_format_report_swot(
                        get_rows("Testing Report"), get_rows("SWOT"), file_path
                    )
                elif "Games Info" in sheet_names and "Sheet1" in sheet_names:
                    self.parse_format_games_info(
                        get_rows("Games Info"), get_rows("Sheet1"), file_path
                    )
                ...
                print(
                    f"Loaded Excel sheets: {sheet_names} from {os.path.basename(file_path)} (using xlrd)"
                )
    ...
    def parse_format_report_swot(self, report_rows, swot_rows, file_path):
        ...
        for r in report_rows:
            ...
            # Extract evaluations
                ...
                heading = clean_row[1]
                desc = clean_row[3]
                # Filter out garbage and skip helper texts
                if (
                    self.is_valid_row(heading, desc)
                    and len(desc) > 10
                    and not desc.startswith("Yêu cầu")
                    and not desc.startswith("Ví dụ")
                ):
        ...
        for r in swot_rows:
            ...
            if current_section and len(clean_row) > 1 and clean_row[1]:
                ...
                if (
                    len(content) > 15
                    and "What are" not in content
                    and "List down" not in content
                    and "Why is this" not in content
                ):
    ...
    def parse_format_games_info(self, info_rows, sheet1_rows, file_path):
        ...
        # Process SWOT
        if swot_comment:
            swot_sections = re.split(
                r"(\d+/\s*(?:Strengh|Weakness|Oppotunities|Threats):?)", swot_comment
            )
        ...
        for r in sheet1_rows:
            ...
            if len(clean_row) > 3 and clean_row[1] and clean_row[3]:
                ...
                desc = clean_row[3]
                if (
                    self.is_valid_row(feat, desc)
                    and len(desc) > 15
                    and feat != "Kỹ năng"
                    and feat != "Thời trang"
                    and "Mô tả" not in desc
                ):
    ...
    def parse_format_overview_detail(self, overview_rows, detail_rows, file_path):
        ...
        project_name = re.sub(
            r"(_notes|_eval|_review|_evaluation|sample|đánh giá game|\[.*?\]|\(.*?\))",
            "",
            base_name,
            flags=re.IGNORECASE,
        )
        project_name = (
            project_name.replace(".xlsx", "")
            .replace(".xls", "")
            .replace("_", " ")
            .replace("-", " ")
            .strip()
    ...
    def parse_format_generic(self, rows, file_path):
        ...
        for r in clean_rows[start_row:]:
            ...
            if any(
                k in category.lower() for k in ["dự án", "tên game", "project name", "game title"]
            ):
            ...
            if any(k in category.lower() for k in ["đối thủ", "competitors"]):
                competitors = [
                    c.strip() for c in (comment or category.split(":")[-1]).split(",") if c.strip()
                ]
                continue
        ...
        if not project_name:
            ...
            clean_name = re.sub(
                r"(_notes|_eval|_review|_evaluation|sample|đánh giá)",
                "",
                name_without_ext,
                flags=re.IGNORECASE,
            )
    ...
    def _run_cmd(self, cmd, timeout=10):
        """Runs a shell command and returns stdout, or empty string on failure."""
        try:
    ...
    def _is_tool_available(self, tool_name):
        """Check if a CLI tool is installed and responding."""
        try:
            result = subprocess.run(
                f"{tool_name} --version",
                shell=True,
    ...
    def _search_exa(self, query, num_results=5):
        """Search via Exa AI engine (mcporter)."""
        print(f"  [Exa] Searching: {query}")
        raw = self._run_cmd(
            f"mcporter call 'exa.web_search_exa(query: \"{query}\", numResults: {num_results})'",
            timeout=15,
        )
        if not raw or "Unknown MCP server" in raw:
            return []
        try:
            data = json.loads(raw)
            results = []
    ...
    def _search_reddit(self, query, limit=5):
        """Search Reddit for user discussions and reviews."""
        print(f"  [Reddit] Searching: {query}")
        raw = self._run_cmd(f'rdt search "{query}" --limit {limit}', timeout=15)
        if not raw or "forbidden" in raw.lower() or "error" in raw.lower():
            return []
        results = []
    ...
    def _search_twitter(self, query, limit=5):
        """Search Twitter/X for real-time buzz."""
        print(f"  [Twitter] Searching: {query}")
        raw = self._run_cmd(f'twitter search "{query}" -n {limit}', timeout=15)
        if not raw or "not_authenticated" in raw or "error" in raw.lower():
            return []
        results = []
        for line in raw.split("\n"):
            line = line.strip()
    ...
    def _search_youtube(self, query, limit=3):
        """Search YouTube for video reviews and gameplay."""
        print(f"  [YouTube] Searching: {query}")
        raw = self._run_cmd(f'yt-dlp --dump-json "ytsearch{limit}:{query}"', timeout=20)
        if not raw:
            return []
        results = []
        for block in raw.split("\n"):
            ...
            try:
                ...
                if title:
                    results.append(f"[{channel}] {title} ({views:,} views) — {desc}")
            except (json.JSONDecodeError, TypeError):
                continue
        return results[:limit]

    def _search_bing(self, query, max_results=5):
        """Search via DuckDuckGo (free, no auth required)."""
        ...
        try:
            ...
            for r in results:
                ...
                body = r.get("body", "")
                if title or body:
                    formatted.append(f"{title} — {body}" if title else body)
            return formatted[:max_results]
        except Exception as e:
            print(f"  [DDG] Search failed: {e}")
            return []

    def search_web_info(self, max_results=5):
        """Multi-source search across available channels."""
        name = self.project_name
        genre = self.genre

        queries = {
            "gameplay_review": f"{name} gameplay review {genre}",
            "monetization": f"{name} monetization gacha model",
            "sentiment": f"{name} player reviews opinions 2026",
            "revenue": f"{name} revenue downloads market data",
        }
        if self.competitors:
            queries["competitors"] = f"{name} vs {self.competitors[0]} comparison"
        ...
        for category, query in queries.items():
            ...
            jina = self._search_bing(query, max_results=max_results)
            if jina:
                category_results.extend([f"[Web] {r}" for r in jina])

            exa = self._search_exa(query, num_results=max_results)
            if exa:
                category_results.extend([f"[Exa] {r}" for r in exa])

            all_results[category] = category_results

        self.web_research_results = all_results
        return all_results

    def research(self):
        """Multi-source market research with categorized results."""
        ...
        if self.competitors:
            research_steps.append(
                f"So sánh các tính năng cốt lõi với đối thủ: {', '.join(self.competitors)}..."
            )
        else:
            research_steps.append("Không phát hiện đối thủ cạnh tranh cụ thể để so sánh.")

        all_results = self.search_web_info(max_results=4)

        category_labels = {
            "gameplay_review": "🎮 Gameplay & Reviews",
            "monetization": "💰 Monetization Model",
            "sentiment": "💬 Player Sentiment",
            "revenue": "📈 Revenue & Market Data",
            "competitors": "⚔️ Competitor Comparison",
        }

        for category, items in all_results.items():
            label = category_labels.get(category, category.title())
            if items:
                research_steps.append(f"\n### {label}")
                for item in items:
                    research_steps.append(f"- {item}")
            else:
                research_steps.append(f"\n### {label}\n- Không tìm thấy dữ liệu cho danh mục này.")

        if not any(all_results.values()):
            research_steps.append(
                "Không thể thu thập thông tin Internet. Dữ liệu dựa trên phân tích nội bộ."
            )
    ...
    def write_slides_report(self, scorecard, research_data, output_dir):
        ...
        report_path = os.path.join(
            output_dir, f"Evaluation_{safe_project_name}_{datetime.now().strftime('%Y%m%d')}.md"
        )
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"**Ngày tổng hợp:** {datetime.now().strftime('%Y-%m-%d')} | **Thể loại:** {self.genre}\n\n"
            )
            ...
            f.write(
                f"- **Đối thủ chính:** {', '.join(self.competitors) if self.competitors else 'Không có (N/A)'}\n"
            )
            f.write(f"- **Chỉ số đánh giá trung bình:** {avg_score}/5.0\n")
            f.write(
                "- **Tóm tắt nhanh:** Báo cáo được phân tích tự động từ dữ liệu ghi chú Excel và phân tích cạnh tranh của AI Agent.\n\n"
            )
            ...
            f.write(
                "- Cần tập trung cải thiện các điểm hạn chế đã ghi nhận trong phần phân tích chi tiết.\n"
            )
            f.write(
                "- Chuẩn bị các phương án vận hành linh hoạt dựa theo đặc thù thị trường Việt Nam.\n\n"
            )
            ...
            f.write(
                "1. **Tối ưu hóa sản phẩm:** Sửa đổi các lỗi thiết kế UI/UX và cơ chế game chưa cân bằng.\n"
            )
            f.write(
                "2. **Định hình mô hình kinh doanh:** Điều chỉnh chiến lược nạp IAP và quảng cáo để không ảnh hưởng xấu tới trải nghiệm người dùng.\n"
            )
            f.write(
                "3. **Xuất PowerPoint:** Sử dụng cấu trúc slide Markdown này để đóng gói thành file PPTX thuyết trình chính thức.\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--info", type=str, default="", help="Thông tin chi tiết / ghi chú của game"
    )
    parser.add_argument(
        "--competitors",
        type=str,
        default="",
        help="Danh sách đối thủ cạnh tranh (phân cách bằng dấu phẩy)",
    )
    parser.add_argument(
        "--criteria",
        type=str,
        default="",
        help="Danh sách các tiêu chí đánh giá (phân cách bằng dấu phẩy)",
    )
        ...
        for cr in criteria:
            ...
            if "core" in cr_lower or "loop" in cr_lower:
                ...
                if any(x in info_lower for x in ["nạp", "tiền", "mua", "ads", "quảng cáo", "iap"]):
                    ...
                    comment = "Áp dụng mô hình Hybrid Monetization (kết hợp IAP và quảng cáo xen kẽ). Các điểm chạm mua hàng được sắp đặt tự nhiên, giảm thiểu sự khó chịu cho người chơi miễn phí."
            elif "visual" in cr_lower or "ux" in cr_lower or "đồ họa" in cr_lower:
                if any(
                    x in info_lower for x in ["đồ họa", "hình ảnh", "art", "giao diện", "ui", "ux"]
                ):
                    ...
                    comment = "Phong cách nghệ thuật hiện đại, màu sắc hài hòa và giao diện người dùng tối giản. Các hiệu ứng chuyển động mượt mà hỗ trợ tốt cho cảm giác chơi (game feel)."
            elif "retention" in cr_lower or "giữ chân" in cr_lower:
                comment = "Hệ thống nhiệm vụ hàng ngày, sự kiện tuần và cơ chế thưởng đăng nhập hoạt động hiệu quả để giữ chân người chơi lâu dài."
            elif "usp" in cr_lower or "điểm đặc biệt" in cr_lower:
                comment = "Điểm độc đáo nằm ở việc tối ưu hóa hiệu năng cực tốt và kết hợp các yếu tố gameplay sáng tạo so với các đối thủ cùng thể loại trên thị trường."
            else:
            ...
            # positive keywords
            if any(
                w in comment_lower or w in info_lower
                for w in [
                    "tốt",
                    "mượt",
                    "đẹp",
